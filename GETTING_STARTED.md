...
> ⚠️ **版本提示**：推荐 **Python 3.11 或 3.12**。不要使用 3.13+（与 pywebview 等依赖不兼容）。3.14 尤其不稳定，踩过坑。
...
> 💡 **注意**：`mykey.py` 已在 `.gitignore` 中，不会被 git 追踪，密钥不会意外上传。
...
> 💡 **个人备注**：我用的是 `oai_config` 搭配本地 Ollama（`apibase` 填 `http://localhost:11434/v1`），完全离线可用，隐私友好。
